...
const FleetManager = () => {
  ...
  // MQTT hook for live fleet updates
  ...
  // Handle MQTT fleet updates
  useEffect(() => {
    if (latestTruckUpdate) {
      console.log('Fleet Manager: Received MQTT update:', latestTruckUpdate);
      setFleetData(prevFleetData => {
        const updatedFleet = prevFleetData.map(truck =>
          truck._id === latestTruckUpdate._id ? {
            ...truck,
            truck: {
              ...truck.truck,
              batterySOC_percent: latestTruckUpdate.truck?.batterySOC_percent ?? truck.truck.batterySOC_percent,
              batteryTemperature_C: latestTruckUpdate.truck?.batteryTemperature_C ?? truck.truck.batteryTemperature_C,
              chargingStatus: latestTruckUpdate.truck?.chargingStatus ?? truck.truck.chargingStatus
            }
          } : truck
        );
        return updatedFleet;
  ...
  const fetchData = async () => {
    try {
      ...
      if (liveData.success) {
        ...
        // Fetch route geometries for live routes
        fetchRouteGeometries(liveData.liveRoutes);
      }
  ...
  const fetchRouteGeometries = async (routes) => {
    const geometries = {};
    for (const route of routes) {
      try {
        ...
        if (routeData.features?.[0]?.geometry?.coordinates) {
          ...
          geometries[route.routeId] = routeData.features[0].geometry.coordinates.map(coord => [coord[1], coord[0]]);
        }
      } catch (error) {
    ...
    <div style={{ padding: '20px', backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '30px' }}>
        ...
        <div style={{ ...cardStyle, textAlign: 'center' }}>
          ...
          <p style={{ margin: 0, color: '#666' }}>High Battery (Live)</p>
        </div>
        <div style={{ ...cardStyle, textAlign: 'center' }}>
          <h2 style={{ color: '#ff9800', margin: '0 0 8px 0' }}>
            {fleetData.filter(f => f.truck && f.truck.chargingStatus === 'charging').length}
          </h2>
          <p style={{ margin: 0, color: '#666' }}>Charging (Live)</p>
        </div>
        <div style={{ ...cardStyle, textAlign: 'center' }}>
      ...
      <div style={{ ...cardStyle, marginBottom: '20px', backgroundColor: latestTruckUpdate ? '#e8f5e8' : '#fff3e0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ 
            width: '8px', 
            height: '8px', 
            borderRadius: '50%', 
            backgroundColor: latestTruckUpdate ? '#4caf50' : '#ff9800' 
          }}></div>
          <span style={{ fontSize: '14px', fontWeight: 'bold' }}>
            MQTT Status: {latestTruckUpdate ? 'Connected - Live Updates Active' : 'Waiting for updates...'}
          </span>
        </div>
        {latestTruckUpdate && (
          <p style={{ margin: '4px 0 0 16px', fontSize: '12px', color: '#666' }}>
      ...
      <div style={{ ...cardStyle, marginBottom: '30px' }}>
        ...
        <div style={{ height: '400px', width: '100%', borderRadius: '8px', border: '1px solid #ddd' }}>
          ...
          >
                      ...
                      <div>
                        ...
                        {fleet.truck && (
                          <>
                            ...
                            <p><strong>Battery SOC:</strong> {fleet.truck.batterySOC_percent}% 
                              <span style={{color: '#4caf50', fontSize: '12px'}}> (Live)</span>
                            </p>
                            <p><strong>Range:</strong> {fleet.truck.range_km} km</p>
                            <p><strong>Status:</strong> {fleet.truck.chargingStatus}
                              <span style={{color: '#4caf50', fontSize: '12px'}}> (Live)</span>
                            </p>
                            <p><strong>Temperature:</strong> {fleet.truck.batteryTemperature_C}°C
                              <span style={{color: '#4caf50', fontSize: '12px'}}> (Live)</span>
                            </p>
                          </>
                        )}
            ...
            {/* Live Route Markers and Polylines */}
            {liveRoutes.map((route, index) => {
              ...
              if (route.currentPosition) {
                ...
                markers.push(
                  <Marker key={`live-${route.routeId}-${index}`} position={[lat, lng]} icon={liveMarkerIcon}>
                    <Popup>
                      <div>
              ...
              if (routeGeometries[route.routeId] && routeGeometries[route.routeId].length > 0) {
                polylines.push(
                  <Polyline
                    key={`polyline-${route.routeId}-${index}`}
        ...
        <div style={{ marginTop: '10px', display: 'flex', gap: '20px', fontSize: '14px', color: '#666' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
              ...
              <div key={`${route.routeId}-${index}`} style={cardStyle}>
                ...
                <div style={{ marginTop: '12px' }}>
                  ...
                  <div style={progressBarStyle}>
                    <div style={progressFillStyle(route.progress || 0)}></div>
                  </div>
                </div>
        ...
        <div style={{ ...cardStyle, height: '400px', overflow: 'auto' }}>
          <h2 style={{ margin: '0 0 20px 0', color: '#333' }}>
            Fleet Status ({fleetData.length}) - Live MQTT Updates
          </h2>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  {fleet.truck && (
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '10px', color: '#4caf50' }}>●LIVE</span>
                    </div>
                  )}
                </div>
                  ...
                  <>
                    ...
                    <div style={{ marginTop: '12px' }}>
                      <p style={{ margin: '0 0 4px 0', fontSize: '14px' }}>
                        ...
                        <span style={{ fontSize: '10px', color: '#4caf50', marginLeft: '4px' }}>●LIVE</span>
                      </p>
                      <div style={progressBarStyle}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '12px' }}>
                      ...
                      <p style={{ margin: 0, fontSize: '14px' }}>
                        ...
                        <span style={{ fontSize: '10px', color: '#4caf50', marginLeft: '2px' }}>●</span>
                      </p>
                    </div>
                    <p style={{ margin: '8px 0 4px 0', fontSize: '14px' }}>
